...
def singleLQvk0(submission):
    #Prepare
    table = Table("Acceptance times efficiency for single vector leptoquarks, k = 0")
    table.location = ""
    table.description = "Single vector leptoquark (LQv) k = 0 total selection efficiency, accounting for both the decay branching fraction and the event selection, for events that pass the signal region requirements and any of the top quark or b jet categories defined in the search."
    data = np.loadtxt("input/singleLQvk0.txt", skiprows=2)

    #Indepedent variable (x axis)
    resonance_mass = Variable("LQv k = 0 mass", is_independent=True, is_binned=False, units="GeV")
    ...
    eff_LQl1p0.add_qualifier("Process", "LQv k = 0 $\lambda$ = 1.0")
    ...
    eff_LQl1p5.add_qualifier("Process", "LQv k = 0 $\lambda$ = 1.5")
    ...
    eff_LQl2p0.add_qualifier("Process", "LQv k = 0 $\lambda$ = 2.0")
    ...
    eff_LQl2p5.add_qualifier("Process", "LQv k = 0 $\lambda$ = 2.5")
